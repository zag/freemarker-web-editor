define([], function () {
    CodeMirror.commands.autoComplete = function (cm) {
        CodeMirror.simpleHint(cm, CodeMirror.freemarkerHint);
    }
    ...
    CodeMirror.simpleHint = function (editor, getHints, givenOptions) {
        ...
        function collectHints(previousToken) {
            ...
            var completions = getHints(editor);
            if (!completions /*|| !completions.length*/) return;

            function insert(suggestion, replace) {
                var curFrom = suggestion.getFrom();
                var curTo = suggestion.getTo();

                if (!replace) {
                    editor.replaceRange(suggestion.getText(), suggestion.getFrom(), suggestion.getTo());
                }
                else {
                    if (suggestion.getFrom().ch < suggestion.getTo().ch) {
                        var token = editor.getTokenAt(suggestion.getTo());
                        curFrom.ch = token.start;
                        curTo.ch = token.end;
                    }

                    editor.replaceRange(suggestion.getText(), curFrom, curTo);
                }

                // Initiating tag close on inserted suggestion
                if (CodeMirror.commands.autoCloseTag !== undefined) {
                    var inserted = suggestion.getText();
                    if (inserted.charAt(inserted.length - 1) == ">") {
                        var tagName = inserted.replace(/^<#([a-zA-Z]+)>/, "$1");
                        CodeMirror.commands.autoCloseTag(editor, tagName, curTo);
                    }
                }

                if (suggestion.getSelected()) {
                    var selEnd = {line: curFrom.line, ch: curFrom.ch};
                    selEnd.ch += suggestion.getText().length;
                    editor.setSelection(curFrom, selEnd);
                }

                editor.indentLine(curFrom.line);
            }
            ...
            if (sel.firstChild !== null) {
                ...
                pos = editor.cursorCoords(options.alignWithWord? completions[0].from : null);
            }
            else {
                pos = editor.cursorCoords(null);
            }
            ...
            function pick(replace) {
                ...
                var picked = completions[sel.selectedIndex];
                if (picked.isClickable()) {
                    insert(picked, replace);
                }
